    ...
    \begin{singlespace}
        ...
        \newpage
				\begin{versionhistory}
					...
					\vhEntry{2.0.1}{5.21.18}{\GroupMemberOne}{Updated the UI description to reflect what we actually built. Fixed minor errors.}
					\vhEntry{2.0.2}{5.21.18}{\GroupMemberOne}{Updated the functional requirements section to match what we actually built.}
				\end{versionhistory}
		...
		\subsection{Functional Requirements}
			...
			\subsubsection{Functional Requirements For Climate Data API}
					...
					\paragraph{Functional Requirement 1.2}
						...
						Description: The API shall take the users State and county information and transform it into latitude and longitude using a python library.\\
						Dependencies: FR1.1 and FR2.1\hfill \break

					\paragraph{Functional Requirement 1.3}
						\textbf{ID: FR1.3}\hfill \break
						Title: The data source \hfill \break
						Description: The data we will use for this API is the ensemble mean model. This data will be provided through a local file. This local file will be put in place using a chron job on the prod server. The data will be exposed to the Docker Container using a mount bind.\\
						Dependencies: FR1.1, FR1.2, FR 2.1\hfill \break

					\paragraph{Functional Requirement 1.4}
						\textbf{ID: FR1.4}\hfill \break
						Title: Getting the Data\hfill \break
						After validating the state and county passed to the user the API will then get the data from the local data file. The API will select the closest point with in 20 miles where data is available. If no data is available with in 20 miles the API will return an error.\\
						Dependencies: FR1.1, FR1.2, FR1.3 and FR2.1\hfill \break

					\paragraph{Functional Requirement 1.5}
						\textbf{ID: FR1.5}\hfill \break
						Title: Calculating Historical Data\hfill \break
						Description: After getting the data from the data file we will need to calculate the historical average from the anomaly data and forecast data.\\
					...
					\paragraph{Functional Requirement 1.6}
							...
							Description: Once the we have all the data we need we will format the data in a JSON object. The JSON object will have meaningful labels that will make it clear what is contained in each field.\\
					...
					\paragraph{Functional Requirement 1.7}
						...
						Title: Sending the data\hfill \break
			...
			\subsubsection{Functional Requirements For Front End}
				...
				\paragraph{Functional Requirement 2.1}
					...
					Description: The \textit{AgBizClimate} application the will have a page that will allow the user to select between the short term climate tool and the long term climate tool.\\
				...
				\paragraph{Functional Requirement 2.3}
					...
					Description: After the API processes the request for the data for the location the the user specified the front end will receive the response from the API. The front end shall then consume the data in response body. The data will then be plotted using a plotting library to plot the data and displayed to the user.\\
				...
				\paragraph{Functional Requirement 2.5}
					...
					Description: Once the user has entered the adjustments to the yield for the crop they are analyzing they shall then be redirected to the existing \textit{AgBizClimate} budgeting tool. This tool will be sent the data they entered in the previous step and shall reflect the adjustments to the yield.\\
    ...
    \subsection{Other Requirements}
				...
				In this section we will discuss requirements not apart of our contract with our client. These goals will not be required as apart of this project but are objectives that we would like to complete.\\
					...
					\paragraph{Stretch Requirement 1.2}
						...
						Dependencies: FR1.1, FR1.2, FR1.3, FR1.4, FR1.5 and FR 2.1 \hfill \break
\begin{landscape}
	\section{Gantt Chart}
